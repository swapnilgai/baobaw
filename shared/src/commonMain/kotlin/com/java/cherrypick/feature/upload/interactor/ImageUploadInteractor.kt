...
interface ImageUploadInteractor {
    suspend fun imageUpload(bitmap: Bitmap)
}
...
class ImageUploadInteractorImpl(private val supabaseClient: SupabaseClient): ImageUploadInteractor, Interactor{
    override suspend fun imageUpload(bitmap: Bitmap) {
        withInteractorContext {
            supabaseClient.gotrue.currentUserOrNull()?.id?.let {currentUser ->
                supabaseClient.storage["user"].run {
                    this.upload("$currentUser/myIcon.png", bitmap.toByteArray(), upsert = true)
                }
            }
        }
    }
